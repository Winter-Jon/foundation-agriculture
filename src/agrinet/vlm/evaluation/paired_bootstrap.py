#!/usr/bin/env python3
"""Compare paired scored predictions under a manifest-locked protocol."""
...
def paired_review(
    candidate_path: Path, baseline_path: Path, *, bootstrap_samples: int = 10000, seed: int = 20260816,
) -> dict[str, Any]:
    """Return the paired accuracy review without writing a result file."""
    candidate, baseline = load(candidate_path), load(baseline_path)
    if set(candidate) != set(baseline):
        raise ValueError("candidate and baseline ids differ; paired comparison is invalid")
    ids = sorted(candidate)
    if not ids:
        raise ValueError("empty diagnostic")
    lower, upper = bootstrap(ids, candidate, baseline, bootstrap_samples, seed)
    ...
    return {
        "schema_version": "agrinet.vlm.paired-bootstrap/v1",
        "rows": len(ids),
...
def main() -> None:
    ...
    try:
        result = paired_review(
            args.candidate, args.baseline, bootstrap_samples=args.bootstrap_samples, seed=args.seed,
        )
    except ValueError as exc:
        raise SystemExit(str(exc)) from exc
    args.out.parent.mkdir(parents=True, exist_ok=True)
    args.out.write_text(json.dumps(result, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
    print(json.dumps({"rows": result["rows"], "delta_pp": result["paired_delta_pp"], "lower_pp": result["paired_bootstrap_95pct_ci_pp"][0]}, ensure_ascii=False))
