"""E3.14: conditional Classifier-to-RAG cascade with private escalation."""
...
from agrinet.rag.e313_hcv_cascade import PROMPTS, validate_e313_trajectory
from agrinet.rag.e35_classifier_cascade import ARMS, DELIVERY_FAILURES, ROUTES, recovery_attempt

E314_PROTOCOL = "agrinet.e314-hcv-cascade/v1"

def validate_e314_trajectory(row: dict[str, Any], trajectory: dict[str, Any]) -> None:
    try:
        validate_e313_trajectory(row, trajectory)
    except ValueError as exc:
        raise ValueError(str(exc).replace("E3.13", "E3.14")) from exc

def materialize_e314_source(rows: list[dict[str, Any]], coverage: dict[str, str]) -> list[dict[str, Any]]:
    if len(rows) != 32 or len(coverage) != 6:
        raise ValueError("E3.14 needs 32 rows and six private route witnesses")
    result=[]
    for row in rows:
        private=dict(row.get("private") or {})
        if row["sample_id"] in coverage:
            private["e314_route_coverage"] = coverage[row["sample_id"]].rsplit(":",1)[1]
        result.append({**row,"e39_protocol":E314_PROTOCOL,"teacher_system_prompts":dict(PROMPTS),"private":private})
    return result

def write_e314_manifest(*, source: Path, campaign_id: str, output: Path) -> dict[str, Any]:
    if output.exists(): raise ValueError("E3.14 manifest is immutable")
    rows=[json.loads(line) for line in source.read_text().splitlines() if line.strip()]
    if len(rows)!=32 or any(row.get("e39_protocol")!=E314_PROTOCOL for row in rows):
        raise ValueError("E3.14 source binding invalid")
    payload={"schema_version":"agrinet.e314-hcv-cascade-manifest/v1","protocol":E314_PROTOCOL,"campaign_id":campaign_id,"round":"R0","source":str(source),"source_sha256":hashlib.file_digest(source.open("rb"),"sha256").hexdigest(),"source_rows_expected":32,"audit_only":True,"work_items":[{"work_id":f"R0:{row['sample_id']}:e314-hcv","round":"R0","sample_id":row["sample_id"],"image_group_id":row.get("image_group_id",row["image_sha256"]),"attempt_ordinal":0,"predecessor_request_id":None,"route_progression":["direct","classifier","rag"]} for row in rows],"workers":4,"micu_intent_limit":8000,"automatic_replay_allowed":False,"collection_controls":{"uncached_input_token_cap":300000,"transport_image_max_side":1024,"max_public_turns_per_route":12,"max_rag_searches":3,"reservation_uncached_tokens":{"generation":{"direct":5000,"classifier":8000,"rag":12000},"private_audit":18000}},"training_eligible":False,"training_authorized":False,"sft_may_start":False}
    output.parent.mkdir(parents=True,exist_ok=True)
    output.write_text(json.dumps(payload,ensure_ascii=False,indent=2,sort_keys=True)+"\n")
    return payload

def write_e314_replenishment_manifest(*, summary: dict[str, Any], next_round: str, output: Path) -> dict[str, Any]:
    prior={"R1":"R0","R2":"R1"}
    if next_round not in prior or summary.get("round") != prior[next_round]: raise ValueError("E3.14 recovery must proceed R0 -> R1 -> R2")
    if output.exists(): raise ValueError("E3.14 replenishment destination is immutable")
    controls=summary.get("collection_controls")
    if not isinstance(controls,dict) or controls.get("max_rag_searches")!=3 or controls.get("max_public_turns_per_route")!=12: raise ValueError("E3.14 recovery has no frozen collection controls")
    work=[]
    for row in summary.get("rows",[]):
        if row.get("delivery_status") in DELIVERY_FAILURES:
            ...
            if rec.get("new_attempt"): work.append({**{key:row[key] for key in ("sample_id","image_group_id","route_progression")},"work_id":f"{next_round}:{row['sample_id']}:e314-hcv","round":next_round,**rec})
    payload={"schema_version":"agrinet.e314-hcv-cascade-manifest/v1","protocol":E314_PROTOCOL,"campaign_id":summary.get("campaign_id"),"round":next_round,"audit_only":True,"work_items":work,"workers":4,"micu_intent_limit":8000,"automatic_replay_allowed":False,"collection_controls":controls,"training_eligible":False,"training_authorized":False,"sft_may_start":False}
...
def e314_layered_audit_report(*, source_rows: list[dict[str, Any]], summaries: list[dict[str, Any]]) -> dict[str, Any]:
    if len(source_rows)!=32 or [item.get("round") for item in summaries] != ["R0","R1","R2"]:
        raise ValueError("E3.14 final audit needs its 32-row R0/R1/R2 lineage")
    by_id={str(row.get("sample_id")):row for row in source_rows}
    if len(by_id)!=32: raise ValueError("E3.14 audit source has duplicate IDs")
    latest={str(row.get("sample_id")):row for row in summaries[0].get("rows",[])}
    if set(latest)!=set(by_id): raise ValueError("E3.14 R0 summary does not close source scope")
    for summary in summaries[1:]:
        ...
        if expected != observed: raise ValueError("E3.14 recovery summary scope is not exact")
    ...
    for sid,row in latest.items():
        ...
        if row.get("delivery_status") in DELIVERY_FAILURES:
            ...
            terminal["accepted"]+=1
            target=(by_id[sid].get("private") or {}).get("e314_route_coverage")
    ...
    return {"schema_version":"agrinet.e314-audit-final-report/v1","protocol":E314_PROTOCOL,"rows":32,"rounds":["R0","R1","R2"],"terminal_counts":dict(terminal),"final_routes":dict(routes),"coverage_winners":coverage_winners,"protocol_observed":observed,"rag_evidence_closed":rag_evidence_closed,"protocol_gate_passed":gate,"full_campaign_remaining_candidates":878,"full_campaign_expansion":"requires_separate_manifest" if gate else "not_authorized","training_eligible":False,"training_authorized":False,"sft_may_start":False}
...
def main(argv: list[str] | None = None) -> int:
    ...
    if argv and argv[0] == "plan-replenishment":
        ...
        result=write_e314_replenishment_manifest(summary=json.loads(args.summary.read_text()),next_round=args.next_round,output=args.output)
        print(json.dumps({"round":result["round"],"rows":len(result["work_items"])})); return 0
    if argv and argv[0] == "audit-final-report":
        ...
        parser.add_argument("--output",type=Path,required=True)
        args=parser.parse_args(argv[1:])
        if args.output.exists(): raise ValueError("E3.14 audit report destination is immutable")
        ...
        report=e314_layered_audit_report(source_rows=source,summaries=summaries)
        ...
        print(json.dumps({"protocol_gate_passed":report["protocol_gate_passed"]})); return 0 if report["protocol_gate_passed"] else 2
    parser=argparse.ArgumentParser(description=__doc__)
    ...
    if any(path.exists() for path in (args.coverage_output,args.source_output,args.manifest_output)):
        raise ValueError("E3.14 preparation destinations are immutable")
    ...
    rows=select_e311_audit(candidates,prior_rows=prior,seed="e314-audit-v1")
    coverage=select_e310_coverage(rows,seed="e314-coverage-v1")
    source=materialize_e314_source(rows,coverage)
    ...
    args.source_output.parent.mkdir(parents=True,exist_ok=True); args.source_output.write_text("".join(json.dumps(row,ensure_ascii=False,sort_keys=True)+"\n" for row in source))
    manifest=write_e314_manifest(source=args.source_output,campaign_id=args.campaign_id,output=args.manifest_output)
