"""Prepare E3.35, a fresh full Classifier lineage with parent-bound Q1 selection."""
...
PROTOCOL = "agrinet.e335-classifier-full/v1"
CAMPAIGN_ID = "e335-classifier-full-v1"


def prepare(**kwargs: Any) -> dict[str, Any]:
    root = Path(kwargs["output_root"])
    if "e335-classifier-full-v1" not in str(root):
        raise ValueError("E3.35 requires its dedicated fresh artifact root")
    return _prepare(**kwargs, protocol=PROTOCOL, campaign_id=CAMPAIGN_ID,
                    provenance_new="new_e335", provenance_reused="reused_e322_v3",
                    provenance_key="e335_provenance",
                    schema="agrinet.e335-classifier-full-manifest/v1",
                    shard_schema="agrinet.e335-classifier-full-shard-manifest/v1",
                    prepare_audit_schema="agrinet.e335-classifier-full-prepare-audit/v1")
