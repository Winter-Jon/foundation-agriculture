"""Prepare E3.36, a fresh full Classifier lineage with bounded delivery exposure."""
...
PROTOCOL = "agrinet.e336-classifier-full/v1"
CAMPAIGN_ID = "e336-classifier-full-v1"
TERMINAL_UNKNOWN_DELIVERY_MAX_FRACTION = 0.10


def prepare(**kwargs: Any) -> dict[str, Any]:
    root = Path(kwargs["output_root"])
    if "e336-classifier-full-v1" not in str(root):
        raise ValueError("E3.36 requires its dedicated fresh artifact root")
    return _prepare(**kwargs, protocol=PROTOCOL, campaign_id=CAMPAIGN_ID,
                    provenance_new="new_e336", provenance_reused="reused_e322_v3",
                    provenance_key="e336_provenance",
                    schema="agrinet.e336-classifier-full-manifest/v1",
                    shard_schema="agrinet.e336-classifier-full-shard-manifest/v1",
                    prepare_audit_schema="agrinet.e336-classifier-full-prepare-audit/v1",
