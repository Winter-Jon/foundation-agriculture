...
def convert_vloom_results(
    ...
    from agrinet.data.io import write_jsonl_atomic
    ...
    for key, row in payload.items():
        if not row:
            rejected += 1
            continue
        template_vars = row.get("template_vars") or row.get("metadata", {}).get("template_vars", {})
        mapping = build_name_mapping(template_vars, lang=language)
        trace = extract_teacher_trace(key, row, build_name_mapping(template_vars, lang="en"))
        if trace is None:
            rejected += 1
            continue
        legacy = convert_to_student_sft(trace, key, lang=language, name_mapping=mapping if language == "zh" else None)
        if not legacy or _think_len(legacy) < min_think_len:
            rejected += 1
            continue
    ...
    if not converted:
        raise ValueError(f"no valid VLOOM records converted from {input_path}")
    write_jsonl_atomic(output_path, converted)
    return {"accepted": len(converted), "rejected": rejected}
