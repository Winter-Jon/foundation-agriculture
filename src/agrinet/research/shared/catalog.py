...
def isolation_hashes(root: Path = ROOT) -> set[str]:
    """Return image hashes excluded by the historical evidence boundary.

    The active M1 and HCV routes need this read-only calculation, while the
    historical reconstructive builder that originally owned it is archived.
    The source paths and exclusion semantics intentionally remain unchanged.
    """
    from agrinet.data.rebuild_sft import image_digest

    diagnostic = root / (
        "outputs/artifacts/datasets/agrinet-rag-sft-stagea-validation-v3/"
        "matched_diagnostic_192.jsonl"
    )
    hashes = {
        image_digest(str(row.get("image_path") or ""), root)
        for row in read_jsonl(diagnostic)
    }
