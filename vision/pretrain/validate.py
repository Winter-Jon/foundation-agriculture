...
try:
    from utils.metrics import compute_long_tail_metrics
except ImportError:
    compute_long_tail_metrics = None

_logger = logging.getLogger('validate')


# First parser for the config file only
...
# Extended arguments
parser.add_argument('--enable-long-tail-metrics', action='store_true', default=False,
                    help='Enable long-tail metrics')
parser.add_argument('--save-cross-entropy', action='store_true', default=False,
                    help='Save cross entropy scores')
...
def validate(args):
    ...
    # Optimization: Keep predictions on GPU for global metrics
    collect_all_preds = args.metrics_avg or args.enable_long_tail_metrics
    all_preds = []
    all_targets = []
    all_keys = []
    
    # Optimization: Chunked cross-entropy saving
    cross_entropy_chunks = [] if args.save_cross_entropy else None
    ...
    with torch.inference_mode():
        ...
        for batch_idx, (keys, input, target) in enumerate(loader):
            ...
            # Accumulate predictions (GPU optimization)
            if collect_all_preds or args.save_cross_entropy:
                ...
                if args.save_cross_entropy:
                    ...
                    batch_data = np.column_stack((batch_indices, ce_np, p_np, t_np))
                    cross_entropy_chunks.append(batch_data)
    ...
    # Optimization: Transfer accumulated predictions to CPU once at the end
    if collect_all_preds and all_preds:
        _logger.info("Transferring predictions to CPU for metrics...")
        final_preds = torch.cat(all_preds).cpu()
        final_targets = torch.cat(all_targets).cpu()
        ...
        if args.metrics_avg:
            precision = precision_score(final_targets_np, final_preds_np, average=args.metrics_avg, zero_division=0)
            recall = recall_score(final_targets_np, final_preds_np, average=args.metrics_avg, zero_division=0)
            f1 = f1_score(final_targets_np, final_preds_np, average=args.metrics_avg, zero_division=0)
            metric_results = {
        ...
        if args.enable_long_tail_metrics and compute_long_tail_metrics is not None:
            _logger.info("Calculating long-tail metrics...")
            long_tail_metrics = compute_long_tail_metrics(final_preds, final_targets, num_classes=args.num_classes)
            long_tail_results = {
                'balanced_accuracy': round(100 * long_tail_metrics.get('balanced_accuracy', 0.0), 4),
                'macro_f1': round(100 * long_tail_metrics.get('macro_f1', 0.0), 4),
            }
