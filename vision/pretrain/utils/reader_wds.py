...
Hacked together by 2025 Winter Jon
"""
import io
import json
import logging
...
def iterable_image_dataset_iter(self):
    """ Custom __iter__ for IterableImageDataset to use our ReaderWds logic
    """
    for key, img, target in self.reader:
        if self.transform is not None:
            img = self.transform(img)
        if self.target_transform is not None:
            target = self.target_transform(target)
        yield key, img, target
...
def _decode(
    ...
    * pass through JSON byte string (if it exists) without parse
    ...
    if alt_label:
        ...
        class_label = str(meta[alt_label])
        if class_label < 0:
            # skipped labels currently encoded as -1, may change to a null/None value
            return None
    else:
        class_label = str(sample[target_key].decode('utf-8'))
...
class ReaderWds(Reader):
    ...
    def __iter__(self):
        ...
        for sample in ds:
            ...
            yield sample['__key__'], sample['image'], int(target)
            i += 1
