...
cmd=['.venv/bin/python','-m','tools.rag_distill.run_pilot','--plan-file',plan,'--candidate-source','outputs/vlm_data/disease_pest_large/contrast_samples_vit_base.jsonl','--limit','100000','--rag-api','http://127.0.0.1:8077','--output-dir',f'{p}/candidates/{name}','--model','gpt-5.6-luna','--max-tool-turns','3','--max-concurrent','4','--teacher-timeout','180','--teacher-retries','3']
subprocess.run(cmd,env=env,check=True)
selection=subprocess.run(['.venv/bin/python','-m','tools.rag_distill.select_recovery_pilot','--pilot-dir',p])
subprocess.run(['.venv/bin/python','-m','tools.rag_distill.validate_semantic_quality','--train-file',f'{p}/accepted/pilot_train_review.jsonl','--report',f'{p}/review/semantic_validation.json'])
