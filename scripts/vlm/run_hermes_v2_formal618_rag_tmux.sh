#!/usr/bin/env bash
# Formal v2 618-row Hermes RAG candidate/base comparison.
set -euo pipefail
...
LOG="outputs/runs/vlm/vlm-sft-qwen3vl4b-hermes-long-direct-blind-rag-1to1-e5-v2/formal618-rag.log"
...
run() {
  ...
  MODEL_PATH="$model" MANIFEST="$MANIFEST" OUT_DIR="$out" CUDA_VISIBLE_DEVICES="0,1" \
...
run candidate "$CANDIDATE" "$OUT_ROOT/candidate_rag"
run raw_base models/Qwen3-VL-4B-Instruct "$OUT_ROOT/base_rag"
