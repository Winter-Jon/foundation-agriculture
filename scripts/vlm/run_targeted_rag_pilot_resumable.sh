...
mkdir -p "$OUT_DIR"
"$PYTHON_BIN" -m tools.rag_distill.run_pilot --preflight-only --preflight-image --plan-file "$PLAN" --candidate-source "$CANDIDATE_SOURCE" --limit 1 --teacher-timeout 30 --model "$MODEL" --reasoning-effort "$REASONING_EFFORT" >"$OUT_DIR/preflight.json"
