#!/usr/bin/env zsh
# Fresh, hash-isolated large-008 deficit collection.
set -euo pipefail
...
LOG_DIR="outputs/runs/rag/hermes-v2-large008-targeted-supplement/logs"
mkdir -p "$LOG_DIR"
...
direct_pid=$!
( for label in open-en-disease open-en-pest open-zh-disease open-zh-pest option-en-pest option-zh-pest; do run rag "$label"; done ) &
rag_pid=$!
wait $direct_pid $rag_pid
print 'large-008 targeted supplement completed; sample-audit then freeze preflight.'
