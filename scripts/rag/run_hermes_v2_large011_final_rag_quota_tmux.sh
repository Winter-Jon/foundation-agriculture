...
for label in open-zh-disease open-zh-pest option-en-pest; do
  ...
  .venv/bin/python -m tools.rag_distill.collect_hermes_1to1_v2 --targets "$input" --route rag --output-dir "$out" --model gpt-5.6-terra --limit 9999 --request-timeout 120 --rag-api http://127.0.0.1:8077 >"$LOG_DIR/rag-${label}.log" 2>&1 || { rc=$?; [[ $rc == 2 ]] || exit $rc; }
done
