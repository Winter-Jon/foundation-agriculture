#!/usr/bin/env zsh
# Parallel large-007 Blind RAG worker for the already isolated option/zh/pest manifest.
set -euo pipefail
ROOT=$(cd "$(dirname "$0")/../.." && pwd)
cd "$ROOT"
PLAN="outputs/experiments/hermes_long_direct_blind_rag_1to1_v2/large-20260818-large-007/supplement/rag-option-zh-pest.jsonl"
OUTPUT="outputs/experiments/hermes_long_direct_blind_rag_1to1_v2/collection/large-20260818-large-007-supplement-rag-option-zh-pest"
LOG_DIR="outputs/runs/rag/hermes-v2-large007-option-zh-pest-parallel/logs"
mkdir -p "$LOG_DIR"
...
.venv/bin/python -m tools.rag_distill.collect_hermes_1to1_v2 \
  --targets "$PLAN" --route rag --output-dir "$OUTPUT" --model gpt-5.6-terra \
  --rag-api http://127.0.0.1:8077 --limit 9999 --request-timeout 120 \
  >"$LOG_DIR/rag-option-zh-pest.log" 2>&1
