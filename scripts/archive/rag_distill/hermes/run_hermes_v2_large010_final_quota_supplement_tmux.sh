#!/usr/bin/env zsh
# Fresh final per-cell quota repair derived from strict v3 preflight.
set -euo pipefail
...
LOG_DIR="outputs/runs/rag/hermes-v2-large010-final-quota-supplement/logs"
mkdir -p "$LOG_DIR"
...
run() {
  ...
  local argv=(.venv/bin/python -m agrinet.rag.distill.collect_hermes_1to1_v2 --targets "$input" --route "$route" --output-dir "$out" --model gpt-5.6-terra --limit 9999 --request-timeout 120)
  [[ "$route" == rag ]] && argv+=(--rag-api http://127.0.0.1:8077)
  "${argv[@]}" >"$LOG_DIR/${route}-${label}.log" 2>&1 || { local rc=$?; [[ $rc == 2 ]] || return $rc; }
}
( for label in option-en-disease option-en-pest option-zh-disease option-zh-pest; do run direct "$label"; done ) &
direct_pid=$!
( for label in open-en-disease open-en-pest open-zh-disease open-zh-pest option-en-pest; do run rag "$label"; done ) &
rag_pid=$!
wait $direct_pid $rag_pid
print 'large-010 final quota supplement completed; run strict freeze preflight.'
