...
5. **Positive-signal gate.** On a fixed 192-row matched diagnostic, require zero protocol/boundary hard gates, stable autonomous tool use, at least +3 percentage points on the RAG-targeted slice, and a paired-bootstrap 95% lower confidence bound above zero against checkpoint-165. No major language/domain/answer-type subgroup may show an unacceptable regression. Direct-only retention on the same held-out rows has a predeclared tolerance of at least −3pp overall and at least −8pp in each major language/domain/answer-type subgroup. A tiny unpaired aggregate increase is insufficient.
