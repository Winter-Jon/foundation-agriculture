...
- Stage A requires an immutable 32 strict `standard` RAG + 32 current-contract Direct corpus, then a separately approved checkpoint-165 SFT and complete forced-off phase evaluation. Stage B's 16 `stop_correction` rows are deferred and never auto-release.
...
- Deferred Stage-B targets: `outputs/experiments/rag_sft_iteration/approval/rag_expansion_plan_v1/deferred_stop_correction_targets.jsonl`
