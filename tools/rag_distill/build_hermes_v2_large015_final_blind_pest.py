#!/usr/bin/env python3
"""Fresh Blind-only repair for the one remaining open/zh/pest quota row."""
...
DEST = ROOT / "outputs/experiments/hermes_long_direct_blind_rag_1to1_v2/large-20260819-large-015/supplement"
...
def main() -> int:
    ...
    for path in sorted((IMAGE_ROOT / CODE).glob("*")):
        ...
        if digest in forbidden:
            continue
        rows.append({
            "target_id": f"hermes-v2-large015-rag-open-zh-pest-{len(rows) + 1:03d}",
    ...
    write_jsonl(DEST / "rag-open-zh-pest.jsonl", rows)
    report = {"schema_version": "agrinet.hermes-large015-final-blind-pest/v1", "rows": len(rows), "sha256": canonical_json_hash(rows), "forbidden_hashes": len(forbidden), "training_authorized": False}
