...
from agrinet.cli.vlm import assert_single_use_freeze_available, assert_training_authorized, evaluation_root, local_training_env, m1_direct_checkpoint_queue_command, sft_python
...
def test_sft_adapter_accepts_an_explicit_environment_interpreter(tmp_path: Path) -> None:
    config = tmp_path / "train.yaml"
    executable = tmp_path / "python"
    config.write_text("model: fixture\n")
    executable.write_text("#!/bin/sh\n")
    command = MsSwiftAdapter().train_command(config, executable)
    assert command[0] == str(executable)


def test_sft_environment_and_evaluation_root_are_worktree_relative(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    import agrinet.cli.vlm as cli

    executable = tmp_path / ".venv_test" / "bin" / "python"
    executable.parent.mkdir(parents=True)
    executable.write_text("#!/bin/sh\n")
    monkeypatch.setattr(cli, "repository_root", lambda: tmp_path)
    assert sft_python() == executable
    assert evaluation_root("fixture-v1") == tmp_path / "outputs/experiments/vlm-evaluations/fixture-v1"
