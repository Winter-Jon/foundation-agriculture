...
from archive.source.rag_distill.convert_reconstructive_freeze_to_sft import student_messages, student_user_prompt
...
def test_hermes_system_prompt_uses_swift_tool_markup() -> None:
    ...
    assert json.dumps(tools[0], ensure_ascii=False) in prompt
    assert tools_json(tools) not in prompt


def test_swift_hermes_tool_response_is_a_wrapped_user_turn() -> None:
    message = swift_hermes_tool_response_message({"status": "success", "results": []})
    assert message == {
        "role": "user",
...
def test_canonicalizes_served_think_then_tool_call_for_swift_history() -> None:
    served = '<think>Plan retrieval.</think><tool_call>{"arguments":{"query":"leaf spot"},"name":"agrinet_rag_search"}</tool_call>'
    canonical, calls = canonicalize_hermes_tool_turn(
        served, tool_name="agrinet_rag_search", validate_arguments=validate)
    assert calls == [{"name": "agrinet_rag_search", "arguments": {"query": "leaf spot"}}]
    assert canonical == (
        '<think>Plan retrieval.</think><tool_call>\n'
        '{"name": "agrinet_rag_search", "arguments": {"query": "leaf spot"}}\n'
        '</tool_call>'
    )


def test_canonical_tool_turn_rejects_non_planning_prefix() -> None:
    canonical, calls = canonicalize_hermes_tool_turn(
        f"text before {call()}", tool_name="agrinet_rag_search", validate_arguments=validate)
    assert canonical == "" and calls == []
...
def test_oracle_text_is_not_permitted_in_persisted_rag_messages() -> None:
    from archive.source.rag_distill.collect_hermes_1to1_v2 import private_oracle_text_in_messages
...
def test_oracle_preflight_rejects_a_cell_over_its_final_cap(tmp_path) -> None:
    ...
    from archive.source.rag_distill.collect_hermes_1to1_v2 import oracle_capacity_errors
...
def test_public_retrieval_evidence_keeps_names_and_drops_internal_ids() -> None:
    from archive.source.rag_distill.collect_hermes_1to1_v2 import public_retrieval_evidence
...
def test_known_rag_rejection_persists_the_complete_public_trajectory() -> None:
    from archive.source.rag_distill.collect_hermes_1to1_v2 import known_rejection_record
...
def test_oracle_recovery_builder_rejects_anything_but_blind_answer_mismatches(tmp_path) -> None:
    from agrinet.data.sft_recovery import write_jsonl
    from archive.source.rag_distill.build_hermes_1to1_oracle_recovery import main
